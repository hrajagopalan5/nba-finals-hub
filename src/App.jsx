...
const PROP_TYPES = ["Points","Rebounds","Assists","Pts+Reb+Ast","3-Pointers Made","Blocks","Steals"];
const TIER_CFG = {
...
function buildPrompt(mode, gameNum, customLegs, numLegs, juiceLevel) {
  ...
  const juiceCtx = juiceLevel==="conservative"
    ? "CONSERVATIVE approach: target a combined parlay payout in the +200 to +500 range. Favor reliable, high-floor props (72%+ individual confidence). Lines should be achievable — volume scorers at safe numbers, durable rebound/assist props. The goal is the most likely-to-hit parlay within that payout band."
    : juiceLevel==="balanced"
    ? "BALANCED approach: target a combined parlay payout in the +500 to +1000 range. Mix safe anchors with a couple of higher-variance value legs (55-70% individual confidence). Solid risk/reward — willing to reach a bit for the bigger payout."
    : juiceLevel==="longshot"
    ? "LONG SHOT approach: target a combined parlay payout of +1000 or higher. Stack value and ambitious legs (40-58% individual confidence). Bold lines, correlated upside, accept real variance for a big score."
    : "EXTREME approach: target a combined parlay payout of +5000 to +20000 (lottery-ticket territory — bet $20 to win thousands, purely for fun). Stack aggressive, ceiling-game props and unlikely-but-possible outcomes (25-45% individual confidence). This is a swing-for-the-fences fun bet — go big, embrace the variance, and lean into spectacular game scripts.";

  const pData = Object.entries(GAME_CONTEXT.players).map(([name,p])=>{
    const vo = PLAYER_VS_OPP[name];
    return `${name} (${p.team} | ${p.pos}):
  - Playoff avg: ${p.plyPPG}pts/${p.plyRPG}reb/${p.plyAPG}ast | ${p.plyFGP}%FG | ${p.ply3PP}%3P
  ...
  - Rest days heading into game: ${restDays} days (${vo?.restBoost?"performs better with rest":"performance neutral on rest"})
  - Matchup context: ${vo?.matchup}
  - Recent trend: ${p.trend}`;
  }).join("\n\n");
...
HOME COURT FACTOR: ${homeTeam} has home court — crowd energy, familiar floor, referee tendencies
REST DAYS: Both teams have ${restDays} days rest since last game

TEAM CONTEXT:
NYK: ${GAME_CONTEXT.nyk.recentForm} | Playoff scoring: ${GAME_CONTEXT.nyk.playoffPPG} PPG offense, ${GAME_CONTEXT.nyk.playoffOPPG} PPG defense
SAS: ${GAME_CONTEXT.sas.recentForm} | Playoff scoring: ${GAME_CONTEXT.sas.playoffPPG} PPG offense, ${GAME_CONTEXT.sas.playoffOPPG} PPG defense
H2H THIS SEASON: ${GAME_CONTEXT.h2h}
KEY SERIES MATCHUP: ${GAME_CONTEXT.keyMatchup}
INJURIES: ${GAME_CONTEXT.injuries}

ODDS/JUICE APPROACH: ${juiceCtx}

PLAYER DATA (includes H2H vs opponent, home/away splits, last 3 games, rest day impact, specific defensive matchup):
${pData}

IMPORTANT ANALYSIS FACTORS TO WEIGH:
1. Player's stats specifically against this opponent (H2H avg provided above)
2. Who is guarding them — does the defensive matchup help or hurt?
3. Home vs away performance for this game's location
4. Recent form (last 3 games trend)
5. Rest days — fatigue vs freshness
6. Game number in series — early games tend to be more physical/lower scoring
7. Prop correlation — legs that reinforce each other (e.g. if Brunson scores a lot, Knicks likely win, which boosts Hart assists)`;

  if(mode==="auto") return `${base}

TASK: Generate exactly ${numLegs} SGP prop picks for Game ${gameNum}, organized into three tiers (safe/value/shot).
Distribute picks based on juice level: ${juiceLevel==="conservative"?"mostly safe tier":juiceLevel==="balanced"?"mix of safe and value":juiceLevel==="longshot"?"mostly value and shot tiers":"mostly long-shot/extreme picks"}.

For each pick: player, team, prop (Points/Rebounds/Assists/Pts+Reb+Ast/3-Pointers Made/Blocks/Steals), line (e.g. "Over 26.5 Points"), confidence (35-90), tier (safe/value/shot), reasoning (2-3 sentences citing H2H/matchup/trend data specifically), risk.

Also provide featuredSGP: pick the best 3-leg combination from your picks, explain how they correlate positively, estimated odds string, narrative.
Also gameFlow: 2-sentence game flow prediction.

Respond ONLY valid JSON no markdown:
{"gameFlow":"...","picks":[{"player":"...","team":"NYK|SAS","prop":"...","line":"...","confidence":75,"tier":"safe|value|shot","reasoning":"...","risk":"..."}],"featuredSGP":{"legs":["...","...","..."],"estimatedOdds":"+450","narrative":"..."}}`;

  return `${base}

TASK: Evaluate these ${customLegs.length} parlay legs for Game ${gameNum}. Apply the ${juiceLevel} approach when suggesting adjustments.

USER'S LEGS:
${customLegs.map((l,i)=>`${i+1}. ${l.player} (${GAME_CONTEXT.players[l.player]?.team||"?"}) — ${l.prop} ${l.line}`).join("\n")}

For each leg: confidence (0-100), tier (safe 72+/value 58-72/shot <58), reasoning (cite H2H/matchup/home-away data), risk, adjustment (suggest better line or alt prop if confidence below 62, else null).

Assess combined parlay: does correlation help or hurt? Estimated odds. Recommendation: STRONG BET / WORTH A SHOT / PASS. Summary.

Respond ONLY valid JSON:
{"legs":[{"player":"...","prop":"...","line":"...","confidence":72,"tier":"safe","reasoning":"...","risk":"...","adjustment":null}],"parlayAssessment":{"correlation":"...","estimatedOdds":"+380","recommendation":"WORTH A SHOT","summary":"..."}}`;
}
  ...
  return(
    <div onClick={()=>setOpen(o=>!o)} style={{borderRadius:10,border:`1px solid ${open?t.border:"rgba(255,255,255,0.07)"}`,background:open?t.bg:"rgba(255,255,255,0.02)",marginBottom:6,cursor:"pointer",overflow:"hidden",transition:"all 0.2s"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"11px 12px"}}>
        ...
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}><span style={{fontSize:12,fontWeight:700,color:C.text}}>{pick.player}</span><TierBadge tier={pick.tier}/></div>
          <div style={{fontSize:10,color:C.light,fontFamily:"monospace"}}>{pick.line}</div>
        </div>
...
function FeaturedSGP({sgp}){
  return(
    <div style={{borderRadius:12,overflow:"hidden",marginBottom:16,background:"linear-gradient(135deg,rgba(212,168,67,0.18),rgba(0,107,182,0.06))",border:"1px solid rgba(212,168,67,0.5)",boxShadow:"0 0 20px rgba(212,168,67,0.12)"}}>
      <div style={{padding:"14px 16px 10px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
          <div><div style={{fontSize:9,color:C.goldDim,letterSpacing:4,textTransform:"uppercase",fontFamily:"monospace"}}>⚡ FEATURED SGP</div><div style={{fontSize:11,color:C.mid,fontFamily:"monospace",marginTop:2}}>AI-Built Same Game Parlay</div></div>
          <div style={{textAlign:"right"}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:C.gold,textShadow:"0 0 10px rgba(212,168,67,0.55)",letterSpacing:2}}>{sgp.estimatedOdds}</div><div style={{fontSize:8,color:C.goldDim,fontFamily:"monospace",letterSpacing:1}}>EST. ODDS</div></div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:5,marginBottom:10}}>
          {sgp.legs.map((leg,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",borderRadius:6,background:"rgba(0,0,0,0.25)",border:"1px solid rgba(255,255,255,0.06)"}}>
              <div style={{width:16,height:16,borderRadius:"50%",background:"rgba(212,168,67,0.3)",border:"1px solid rgba(212,168,67,0.55)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{fontSize:8,color:C.gold,fontWeight:700}}>{i+1}</span></div>
              <span style={{fontSize:10,color:C.light,fontFamily:"monospace"}}>{leg}</span>
            </div>
          ))}
        </div>
        <div style={{fontSize:10,color:C.light,lineHeight:1.6,fontStyle:"italic"}}>{sgp.narrative}</div>
      </div>
    </div>
  );
}
...
function PicksTab(){
  ...
  const analyze=async()=>{
    ...
    try{
      const res=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:buildPrompt(mode,gameNum,customLegs,numLegs,juiceLevel)})});
      ...
      if(mode==="auto"){setGameFlow(parsed.gameFlow);setResult({picks:parsed.picks||[],featuredSGP:parsed.featuredSGP,mode:"auto"});}
  ...
  const filteredPicks=result?.picks?.filter(p=>activeTier==="all"||p.tier===activeTier);
  const JUICE_OPTIONS=[
    {k:"conservative",label:"Conservative",sub:"+200 to +500",color:"#22C55E"},
    {k:"balanced",    label:"Balanced",    sub:"+500 to +1000",color:C.gold},
    {k:"longshot",    label:"Long Shot",   sub:"+1000 and up",color:"#F97316"},
    {k:"extreme",     label:"Extreme",     sub:"+5000 to +20000",color:"#EF4444"},
  ];
  ...
  return(
    <div>
      ...
      <button onClick={analyze} disabled={loading} style={{width:"100%",padding:"15px",borderRadius:10,border:"1px solid rgba(212,168,67,0.6)",cursor:loading?"not-allowed":"pointer",background:loading?"rgba(212,168,67,0.05)":"linear-gradient(135deg,rgba(212,168,67,0.24),rgba(26,143,227,0.12))",color:loading?C.goldDim:C.gold,fontSize:14,fontFamily:"monospace",letterSpacing:2,textTransform:"uppercase",fontWeight:800,marginBottom:16,transition:"all 0.2s",boxShadow:loading?"none":"0 0 28px rgba(212,168,67,0.18)"}}>
        {loading?"⚡ Claude Opus 4.5 Analyzing...":mode==="auto"?`⚡ Generate ${numLegs}-Leg Game ${gameNum} Picks`:`⚡ Evaluate ${customLegs.length}-Leg ${juiceLevel.charAt(0).toUpperCase()+juiceLevel.slice(1)} Parlay`}
      </button>
      ...
      {result?.mode==="auto"&&(
        <div>
          ...
          {result.featuredSGP&&<FeaturedSGP sgp={result.featuredSGP}/>}
          <div style={{display:"flex",gap:4,marginBottom:10,background:"rgba(0,0,0,0.5)",borderRadius:8,padding:3}}>
            {[["all","All"],["safe","Safe"],["value","Value"],["shot","Long Shot"]].map(([k,lbl])=>(
              <button key={k} onClick={()=>setActiveTier(k)} style={{flex:1,padding:"6px 2px",borderRadius:6,border:"none",cursor:"pointer",background:activeTier===k?"rgba(255,255,255,0.10)":"transparent",color:activeTier===k?(k==="safe"?"#22C55E":k==="value"?C.gold:k==="shot"?"#EF4444":C.text):C.dim,fontSize:9,fontFamily:"monospace",letterSpacing:1,textTransform:"uppercase",fontWeight:activeTier===k?800:500}}>{lbl}</button>
            ))}
          </div>
          {["safe","value","shot"].map(tier=>{
            const picks=filteredPicks?.filter(p=>p.tier===tier);
            if(!picks?.length)return null;
            const t=TIER_CFG[tier];
            return(
              <div key={tier} style={{marginBottom:14}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                  <div style={{fontFamily:"monospace",fontSize:11,fontWeight:800,color:t.color,letterSpacing:2,textTransform:"uppercase"}}>{t.label}</div>
                  <div style={{height:1,flex:1,background:`${t.color}44`}}/>
                </div>
                {picks.map((p,i)=><PickCard key={i} pick={{...p,team:GAME_CONTEXT.players[p.player]?.team||"NYK"}}/>)}
              </div>
            );
          })}
        </div>
