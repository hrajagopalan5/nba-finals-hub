...
const SCHEDULE = [
  ...
  {game:4,date:"Wed, Jun 10",time:"8:30 PM ET",home:"NYK",away:"SAS",status:"next",  network:"ABC"},
...
const PLAYER_VS_OPP = {
  ...
  "V. Wembanyama":   { vsOppPPG:29.0, homeAvg:27.5, awayAvg:29.0, last3avg:29.0, restBoost:true,  matchup:"FINALS: 26/12 G1 (ugly 6-21), 29 G2, then a MONSTER 32/8/6/3blk/2stl in the G3 win — fully arrived. MOMENTUM NOTE: trending sharply UP, just led SAS to its first win. Elite scoring + blocks props; the clear engine of the Spurs' comeback bid." },
...
function useCountdown() {
  ...
  useEffect(()=>{
    const target=new Date("2026-06-11T00:30:00Z");
...
const FINALS_GAMES=[
  ...
  {game:4,date:"Jun 10",time:"8:30 PM ET",home:"NYK",away:"SAS",status:"next",  network:"ABC",awaySeed:2,homeSeed:3},
  ...
  return(
    <div>
      <div style={{borderRadius:14,background:"linear-gradient(135deg,rgba(212,168,67,0.12),rgba(8,12,24,0))",border:"1px solid rgba(212,168,67,0.3)",padding:"16px",marginBottom:14}}>
        ...
        <div style={{marginTop:12,textAlign:"center",fontSize:10,color:C.dim,fontFamily:"monospace"}}>Game 4 · Wed Jun 10 · 8:30 PM ET · Madison Square Garden</div>
      </div>
    ...
    <div style={{borderRadius:10,background:C.card,border:`1px solid ${open?`${accent}55`:"rgba(255,255,255,0.05)"}`,marginBottom:6,overflow:"hidden"}}>
      ...
      {open&&(
        <div style={{padding:"0 12px 12px",borderTop:"1px solid rgba(255,255,255,0.05)"}}>
          ...
          {hasPredData?(<>
          <div style={{marginTop:8,fontSize:8,color:C.gold,fontFamily:"monospace",letterSpacing:2,textTransform:"uppercase",fontWeight:700}}>🔮 Projected · Game 4</div>
...
Key matchup: ${GAME_CONTEXT.keyMatchup}
Injuries: ${GAME_CONTEXT.injuries}
Schedule: Game 1 Jun 3 @SAS (FINAL: NYK won 105-95), Game 2 Jun 5 @SAS (FINAL: NYK won 105-104), Game 3 Jun 8 @NYK (FINAL: SAS won 115-111 — Wembanyama 32/8/6, Castle 23), Game 4 Jun 10 @NYK (UPCOMING — the next game, at Madison Square Garden), then 5-7 if needed.
Series status: NYK leads 2-1. SAS got on the board in Game 3 behind a huge Wembanyama/Castle night. Game 4 is the next game, in New York at MSG — Knicks aim to retake control at 3-1; Spurs aim to even it at 2-2.
