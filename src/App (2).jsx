...
TASK: Generate exactly ${numLegs} SGP prop picks for Game ${gameNum}, organized into three tiers (safe/value/shot). Use a VARIETY of the available prop types — include defense, combos, 3PM, and milestone props where the data supports them, not just points.

For each pick: player, team, prop (from the available types), line (e.g. "Over 26.5 Points" or "ALT Over 32.5 Points"), odds (American, e.g. "-150" or "+260"), confidence (35-90), tier (safe/value/shot), reasoning (2-3 sentences citing specific data + at least one of: pace/blowout/foul/correlation), risk.

Also featuredSGP: best combination of legs, explain positive correlation explicitly, combined American odds, the $${stake} payout, narrative.
Also gameFlow: 2-sentence prediction including expected pace and whether it stays competitive.

Respond ONLY valid JSON, no markdown:
{"gameFlow":"...","picks":[{"player":"...","team":"NYK|SAS","prop":"...","line":"...","odds":"+150","confidence":75,"tier":"safe|value|shot","reasoning":"...","risk":"..."}],"featuredSGP":{"legs":["..."],"estimatedOdds":"+450","payout":"$${stake} → $X","narrative":"..."}}`;
  ...
  return(
    <div style={{borderRadius:12,overflow:"hidden",marginBottom:16,background:"linear-gradient(135deg,rgba(212,168,67,0.18),rgba(0,107,182,0.06))",border:"1px solid rgba(212,168,67,0.5)",boxShadow:"0 0 20px rgba(212,168,67,0.12)"}}>
      <div style={{padding:"14px 16px 10px"}}>
        ...
        {sgp.payout&&<div style={{marginTop:10,padding:"8px 12px",borderRadius:8,background:"rgba(34,197,94,0.1)",border:"1px solid rgba(34,197,94,0.3)",textAlign:"center"}}><span style={{fontSize:9,color:"#22C55E",fontFamily:"monospace",letterSpacing:1,textTransform:"uppercase"}}>💰 Potential Payout: </span><span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:"#22C55E",letterSpacing:1}}>{sgp.payout}</span></div>}
      </div>
    </div>
  );
}
...
function PicksTab(){
  ...
  const copyParlay=()=>{
    let txt="";
    if(result?.mode==="auto"&&result.featuredSGP){
      const s=result.featuredSGP;
      txt=`🏀 NBA Finals Game ${gameNum} — Featured SGP\n${s.legs.map((l,i)=>`${i+1}. ${l}`).join("\n")}\nOdds: ${s.estimatedOdds||""}${s.payout?` (${s.payout})`:""}\n— via NBA Finals Hub`;
    } else if(result?.mode==="custom"&&result.legs){
      txt=`🏀 NBA Finals Game ${gameNum} — My Parlay\n${result.legs.map((l,i)=>`${i+1}. ${l.player} ${l.line} ${l.prop} (${l.odds||""})`).join("\n")}\n${result.assessment?`Odds: ${result.assessment.estimatedOdds} · ${result.assessment.recommendation}`:""}\n— via NBA Finals Hub`;
    }
    if(txt){navigator.clipboard?.writeText(txt).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),2000);}).catch(()=>{});}
  };
    ...
    <div>
      ...
      {result?.mode==="auto"&&(
        <div>
          ...
          {result.featuredSGP&&<button onClick={copyParlay} style={{width:"100%",padding:"10px",borderRadius:8,border:"1px solid rgba(212,168,67,0.4)",background:copied?"rgba(34,197,94,0.15)":"rgba(212,168,67,0.1)",color:copied?"#22C55E":C.gold,fontSize:11,fontFamily:"monospace",letterSpacing:1,fontWeight:700,cursor:"pointer",marginBottom:14,transition:"all 0.2s"}}>{copied?"✓ COPIED TO CLIPBOARD":"📋 COPY PARLAY TO SHARE"}</button>}
